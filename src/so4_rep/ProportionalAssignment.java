...
public class ProportionalAssignment
{
	Random r = new Random();
	...
	public ProportionalAssignment(int framesQuantity, int processesQuantity, int referencesQuantity, ArrayList<Process> processes, 
			ArrayList<Frame> frames)
	{
	...
	public void proportionalAssignment()
	{
		int sum = 0;
		for(int i = 0; i < processesQuantity; i++)
		{
			sum += getProcess(i).pageQuantity;
		}
		for(int j = 0; j < processesQuantity; j++)
		{
			double a = (getProcess(j).pageQuantity * framesQuantity) / sum;
			assignment.add((int) a);
		}
	}
